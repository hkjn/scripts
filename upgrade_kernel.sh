...
cp -r ${LIBS} ${BACKUP}/
cp -r ${LIBS} ${LIBS}-stable/

echo "Backing up previous compiled kernel to ${BACKUP}/${KERNEL}.efi and rotating it to ${KERNEL}-stable.efi.."
cp -v ${KERNEL}.efi ${BACKUP}/
cp -v ${KERNEL}.efi ${KERNEL}-stable.efi

# Back up initial ram image and fallback (initrd).
echo 'Backing up + rotating previous initrd (and fallback) image..'
cp -v ${BOOT}/initramfs-arch.img ${BACKUP}/
cp -v ${BOOT}/initramfs-arch.img ${BOOT}/initramfs-arch-stable.img
cp -v ${BOOT}/initramfs-arch-fallback.img ${BACKUP}/
cp -v ${BOOT}/initramfs-arch-fallback.img ${BOOT}/initramfs-arch-fallback-stable.img
...
mv -v /boot/vmlinuz-linux ${KERNEL}.efi

echo "Copying in initial ram images.."
mv -v /boot/initramfs-linux.img ${BOOT}/initramfs-arch.img
mv -v /boot/initramfs-linux-fallback.img ${BOOT}/initramfs-arch-fallback.img
