...
Forked from https://gitorious.org/willgit/mainline?f=bin/git-wtf;hb=HEAD.
...
def show b
  ...
  puts "\n#{red "WARNING"}: local and remote branches have diverged. A merge will occur unless you rebase." if 
oosync
end
  ...
  if $config["integration-branches"].include? b[:local_branch]
    ...
    fbs.each do |name, br|
      ...
      if local_ahead.empty? && remote_ahead.empty?
        ...
        puts "#{widget false, remote_only, local_only} #{name} #{local_only ? "(local-only) " : ""}is NOT 
merged in (#{ahead_behind_string ahead, behind})"
...
$config = { "integration-branches" => %w(heads/master heads/next heads/edge), "ignore" => [], "max_commits" => 
5 }.merge begin
...
`git show-ref`.split(/[\r\n]+/).each do |l|
  ...
  if ref =~ /^heads\/(.+)$/ # local branch
    ...
    branches[name] ||= {}
    branches[name].merge! :name => name, :remote => remote, :remote_branch => "#{remote}/#{branch}", 
:remote_url => remotes[remote], :ignore => ignore
  end
end
